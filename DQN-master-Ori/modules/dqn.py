...
class DQN(object):
	...
	def regression(self, Tea_bar):
		...
		Stu_bar = self.Cal_stu(self, minibatch)
		choosed = self.Sort_Index(self, Stu_bar, Tea_bar)
		replay = deque()
		...
		counter = 0
		while counter <= 10000:
			samples_index  = np.floor(np.random.random((32,))*10000)
			samples = [replay[int(i)] for i in samples_index]

			# Build the bach states
			self.batch_states = np.asarray( [d[0] for d in samples] )  #observation
			self.value_batch = np.asanyarray([d[5] for d in samples])    # q_values from teacher, regression target
