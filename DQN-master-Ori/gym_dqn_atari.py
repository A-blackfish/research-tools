...
def main():
	...
	test_mode = False
	# if len(sys.argv)>=2:
	saver.restore(session, '/home/server4/PycharmProjects/dqn-master-Reg/gym_results/Robotank_v2/final.ckpt')
	if len(sys.argv)==3:
	...
	while num_frames <= TOTAL_FRAMES+1:
		# collecting samples every Timescope
		agent.restore(Teacher)
		tea_bar = agent.Cal_bar()
		if num_frames + 1 % Timescope == 0:
