...
network8 = 'cnn-ocr-8bit'  # you can change the network here, such as cnn-ocr, cnn-ocr2, cnn-orc-test
network7 = 'cnn-ocr-7bit'
...
chars_index = {"京": 0, "沪": 1, "津": 2, "渝": 3, "冀": 4, "晋": 5, "蒙": 6, "辽": 7, "吉": 8, "黑": 9, "苏": 10, "浙": 11, "皖": 12,
...
def select_all(filepath):
    ...
    for j in range(len(Pname)):
        ...
        for i in range(length):
            try:
                label.append(chars_index[name[i]])
            except:
                label.append(24)
        label_all[j] = label
    return Pname, label_all

def get_file_above(file_list):
    rootdir = file_list
    Pname = []
...
def get_file_lower(D):
    Pname = []
    name_all = []
    for i in range(len(D)):
        x = get_file_above(D[i])
        Pname.append(x)
...
def select_all_T(filepath):
    D = get_file_above(filepath)
    Pname = get_file_lower(D)
    Pname_T = []
...
def Transfer(file):
    ...
    for i in range(len(name_all)):
        ...
        img = cv2.imdecode(np.fromfile(file_path+ name_all[i],dtype=np.uint8),-1)
        img = cv2.resize(img, (120, 30))
        img = np.swapaxes(img, 0, 2)
        img = np.swapaxes(img, 1, 2)
        img_all[i] = img
...
def Run(b):  # a is the length of plate , b is file path
    ...
    global n
    global file_path
    file_path = b  # /home/anny/Documents/省市车牌截图 (2)/新/
    name_all, labe_all = select_all(file_path)
    s = Transfer(name_all)
    pre_name, pre_label = TestRecognizeOne(s)

    m = len(name_all)
    n = len(pre_label)
    label_pre = Apart(pre_label)
    name_pre = Apart_name(pre_name)
    accurace = Accuracy(label_pre, labe_all, m)
...
if __name__ == '__main__':
    global length
    length = 7
    _, arg_params, __ = mx.model.load_checkpoint(network7, 1)
    sym = getnet()

    Run('/media/anny/UUI/车牌种类变化子库/香港出入境车牌/')
    Run('/media/anny/UUI/车牌种类变化子库/香港出入境车牌/')
