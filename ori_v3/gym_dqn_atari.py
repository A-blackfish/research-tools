# written by DUAN
# date:2018/12/24
# to run the Timestep transfer learning in reinforcement learning
...
Teacher = 'Gopher-v0.pkl'
ENV_NAME = 'SpaceInvaders-v0'
TOTAL_FRAMES = 6000000 ## TRAIN
Timescope = 20000
...
def main():
	...
	################################################################################
	agent.restore(Teacher)  # load Teacher's experience 
	tea_bar = agent.Calbar()  # calculate KL bar
	# tranning start
	if (num_frames + 1) > 0*TRAIN_AFTER_FRAMES and (num_frames + 1) % Timescope == 0:
		print ('regression start')
		agent.regression(tea_bar)
	...
	while num_frames <= TOTAL_FRAMES+1:
		...
		if not test_mode:
			action, q_values = agent.action(state, training = True)
		else:
