...
class DQN(object):
	...
	def create_graph(self):
		...
		with tf.name_scope("pick_action"):
			...
			self.target_q_values_reg = tf.placeholder(tf.float32, (32, 18), name='target_values')  # added for regression by DUAN
		...
		# By DUAN, added for regression optimizer
		with tf.name_scope("optimization_regression"):
			self.y = tf.nn.softmax(logits=self.target_q_values_reg / 0.1)
			self.s = tf.nn.softmax(logits=self.q_values)
			self.loss = tf.reduce_sum(self.y * (tf.log(self.y) - tf.log(self.s)))

			qnet_gradients_reg = self.qnet_optimizer.compute_gradients(self.loss, self.qnet.variables())
			for i, (grad, var) in enumerate(qnet_gradients_reg):
				if grad is not None:
					qnet_gradients_reg[i] = (tf.clip_by_norm(grad, 10), var)
			self.optimizer = self.qnet_optimizer.apply_gradients(qnet_gradients_reg)
	...
	def regression(self, tea_bar):
		...
		replay = deque()
		for i in range(50000):     # totally are 50000 samples in teacher's replay memory
		...
		counter = 0
		while counter <= 1000:   # regression training 1000 times
