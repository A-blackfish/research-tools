...
class DQN(object):
	...
	def create_graph(self):
		...
		with tf.name_scope("estimating_future_rewards"):
			...
			self.target_q_values = self.rewards + self.discount_factor*self.next_max_q_values  #target Q_values, we can replace it by a teacher's Q_values
		...
		with tf.name_scope("optimization_step"):
			...
			self.loss = tf.reduce_sum( 0.5*tf.square(quadratic_part) + linear_part )
	...
	def store(self, state, action, reward, next_state, is_terminal, q_values):
		# rewards clipping
		if self.reward_clipping > 0.0:
			reward = np.clip(reward, -self.reward_clipping, self.reward_clipping)
		self.experience_replay.store(state, action, reward, next_state, is_terminal, q_values)

	def store_file(self, state, action, reward, next_state, is_terminal, q_values, TD):  # store the samples into the file, Duan
		# rewards clipping
		if self.reward_clipping > 0.0:
			reward = np.clip(reward, -self.reward_clipping, self.reward_clipping)
		self.experience_replay.store_file(state, action, reward, next_state, is_terminal, q_values, TD)

	def restore_samples(self,log):  # regression method, Duan
		if log == 1:    # load teacher's samples into memory, just once
			self.experience_replay.restore()

	def train_samples(self):	# regression method, Duan
		...
		minibatch = self.experience_replay.sample_file(self.num_training_steps)
		if len(minibatch)==0:
			return

		# Build the bach states
		self.batch_states = np.asarray( [d[0] for d in minibatch] )  #observation
		self.value_batch = np.asanyarray([d[5] for d in minibatch])    # q_values from teacher, regression target
		self.teacher = self.value_batch.reshape(32,18)

		# Perform training
		#----------------------------------------------------------------------------------
		self.session.run(self.qnet_optimize,{self.state: self.batch_states,
											 self.target_q_values:self.teacher})
		# self.qnet_optimize.run(feed_dict={self.state:self.batch_states,
		# 							  self.target_q_values:self.teacher})
		cost = self.session.run(self.loss, {self.state: self.batch_states,
											self.target_q_values:self.teacher})
		cy = self.session.run(self.y, {self.state: self.batch_states,
											self.target_q_values: self.teacher})
		cs = self.session.run(self.s, {self.state: self.batch_states,
											self.target_q_values: self.teacher})
		if self.num_training_steps%10000==0:
			...
			print 'lost:',cost
		#----------------------------------------------------------------------------------
		# scores, _, = self.session.run([self.q_values, self.optimizer],
		# 							  { self.state: self.batch_states,
		# 								self.target_q_values: self.teacher} )  # action in this state
		...
		# 	#str_ = self.session.run(self.summarize, { self.state: self.batch_states,
		# 	#										  self.target_q_values: self.value_batch})
	...
	def action(self, state, training = False):
		...
		if random.random() <= epsilon:
			action = random.randint(0, self.env_size-1)
		else:
			action = self.session.run(self.predicted_actions, {self.state:[state] } )[0]
			q_values = self.session.run(self.q_values, {self.state: [state]})
			q_values = q_values.reshape(self.action_size)
			q_values = list(q_values)
			q_values = q_values[0:self.env_size]
			action = q_values.index(max(q_values))
		q_values = self.session.run(self.q_values, {self.state: [state]})
		# q_values = q_values.reshape(self.action_size)
		# q_values = list(q_values)
		# q_values = q_values[0:self.action_size]
		# print action,  q_values.index(max(q_values))
	...
	def train(self):
		...
		# Build the bach states
		batch_states = np.asarray( [d[0] for d in minibatch] )  #observation
		...
		for i in xrange(self.minibatch_size):
			batch_actions[i, actions[i]] = 1  #action
		batch_rewards = np.asarray( [d[2] for d in minibatch] )  #reward
		batch_newstates = np.asarray( [d[3] for d in minibatch] )  #new observation
		batch_newstates_mask = np.asarray( [not d[4] for d in minibatch] )  #is_done

		# Perform training
		scores, _, = self.session.run([self.q_values, self.qnet_optimize],
									  { self.state: batch_states,  # current state
										self.next_state: batch_newstates,  # next state
										self.next_state_mask: batch_newstates_mask,  # is next state done ?
										self.rewards: batch_rewards,  # rewards
										self.action_mask: batch_actions} )  # action in this state
		if self.num_training_steps % self.target_qnet_update_frequency == 0:
			# Hard update (copy) of the weights every # iterations
			self.session.run( self.hard_copy_to_target )
			# Write logs
	...
	def Cal_TD(self, state, action, reward, next_state, done):
		batch_states = [state]  # observation
		actions = [action]
		batch_actions = np.zeros((1, self.action_size))
		for i in xrange(1):
			batch_actions[i, actions[i]] = 1  # action
		batch_rewards = [reward]  # reward
		batch_newstates = [next_state]  # new observation
		batch_newstates_mask =[not done]  # is_done

		# Perform training
		TD = self.session.run(self.error,
									  {self.state: batch_states,  # current state
									   self.next_state: batch_newstates,  # next state
									   self.next_state_mask: batch_newstates_mask,  # is next state done ?
									   self.rewards: batch_rewards,  # rewards
